...
int maximumTableau(int tab[], int taille) {
    int max = tab[0]; // On suppose que le premier �l�ment est le plus grand
...
void modifierTableauAvecPointeurs(int* tab, int taille) {
    for (int i = 0; i < taille; i++) {
        //*(tab + i) += 10; // Ajoute 10 � chaque �l�ment
        tab[i] += 100;
    }
}
...
int main() {
    ...
    std::cout << "Tableau apres modification avec pointeurs (ajout de 10 � chaque �l�ment) : ";
    afficherTableau(tableau, taille);
...
int main() {
    ...
    int* f = new int;
    *f=500;
        double* j = new double;     *j=5.5;
cout << *f << "| " << f << " | " << j << " | " << *j << " | " << &f << " "<< &j << endl;
    delete f;
    delete j;
    */
/*
...
// CHANGE SIZE OF ARRAY NEW INT[]

/*
#include <iostream>
using namespace std;
int main() {
    // Step 1: Create a dynamic array of size 3
    int* arr = new int[3];
    arr[0] = 10;
    arr[1] = 20;
    arr[2] = 30;
